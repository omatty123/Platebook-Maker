...
PPT_CONTENT_HEIGHT = 40
PPT_CONTENT_WIDTH = 180
PPT_ROWS = 3

TIMELINE_LABEL_Y = 218
...
def draw_round(c, x, top, w, h, fill=False):
    y = _y(top, h)
    c.setLineWidth(0.5)
    ...
    c.roundRect(x, y, w, h, CORNER_RADIUS, stroke=1, fill=int(fill))

def draw_centered(c, text, x, top, w, h, size=FONT_SIZE):
...
def draw_standard_plate(c, n, title, date):
    ...
    draw_centered(c, "Timeline", LEFT_MARGIN, TIMELINE_LABEL_Y,
                  TIMELINE_LABEL_WIDTH, TIMELINE_LABEL_HEIGHT)

    draw_round(c, LEFT_MARGIN, TIMELINE_BOX_Y, TIMELINE_BOX_WIDTH, TIMELINE_BOX_HEIGHT, fill=True)
    ...
    draw_centered(c, "Map", LEFT_MARGIN, MAP_LABEL_Y,
                  MAP_LABEL_WIDTH, MAP_LABEL_HEIGHT)
    draw_round(c, LEFT_MARGIN, MAP_BOX_Y, MAP_BOX_WIDTH, MAP_BOX_HEIGHT, fill=True)
    ...
    draw_centered(c, "Penetrating Questions", LEFT_MARGIN, PQ_LABEL_Y,
                  PQ_LABEL_WIDTH, PQ_LABEL_HEIGHT)
    ...
    draw_centered(c, "Very short answers", VSA_LABEL_X, PQ_LABEL_Y,
                  VSA_LABEL_WIDTH, PQ_LABEL_HEIGHT)

    for i in range(2):
        y = Q_ROW_START_Y + i * 60
        draw_round(c, LEFT_MARGIN, y, Q_NUM_WIDTH, Q_ROW_HEIGHT)
        draw_centered(c, str(i + 1), LEFT_MARGIN, y, Q_NUM_WIDTH, Q_ROW_HEIGHT)
        draw_round(c, LEFT_MARGIN + Q_NUM_WIDTH, y, Q_CONTENT_WIDTH, Q_ROW_HEIGHT, fill=True)
        draw_round(c, 396, y, Q_ANSWER_WIDTH, Q_ROW_HEIGHT, fill=True)

    # Bottom
    draw_round(c, LEFT_MARGIN, CEC_LABEL_Y, CEC_LABEL_WIDTH, CEC_LABEL_HEIGHT)
    draw_centered(c, "Causes / Effects / Connections",
                  LEFT_MARGIN, CEC_LABEL_Y, CEC_LABEL_WIDTH, CEC_LABEL_HEIGHT)

    draw_round(c, NOTES_LABEL_X, CEC_LABEL_Y, NOTES_LABEL_WIDTH, CEC_LABEL_HEIGHT)
    draw_centered(c, "Notes",
                  NOTES_LABEL_X, CEC_LABEL_Y, NOTES_LABEL_WIDTH, CEC_LABEL_HEIGHT)

    draw_round(c, LEFT_MARGIN, BOTTOM_BOX_Y, BOTTOM_LEFT_WIDTH, BOTTOM_BOX_HEIGHT, fill=True)
    draw_round(c, BOTTOM_RIGHT_X, BOTTOM_BOX_Y, BOTTOM_RIGHT_WIDTH, BOTTOM_BOX_HEIGHT, fill=True)
...
def generate(lessons_file, output_pdf):
    ...
    w = c.stringWidth(data["course"], FONT_NAME, 14)
    c.drawString((PAGE_WIDTH - w) / 2, PAGE_HEIGHT - 200, data["course"])

    w = c.stringWidth(data["term"], FONT_NAME, 14)
    c.drawString((PAGE_WIDTH - w) / 2, PAGE_HEIGHT - 230, data["term"])

    c.setFont(FONT_NAME, 12)
    name = "Name:_____________________________________"
    w = c.stringWidth(name, FONT_NAME, 12)
    c.drawString((PAGE_WIDTH - w) / 2, PAGE_HEIGHT - 280, name)

    c.showPage()  # blank verso
